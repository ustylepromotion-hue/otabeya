globalThis.__VINEXT_LAZY_CHUNKS__ = ["advisor/1kh/otby/77/assets/page-vLx9Xps2.js","advisor/1kh/otby/77/assets/layout-segment-context-B9rOiziN.js"];
...
var __basePath$1 = "/advisor/1kh/otby/77";
...
function getDraftSecret() {
	return "4d70fd7f-4fdb-4a16-83a9-cae8a8ca9670";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "7b6bacee-4b7a-4639-8cd3-d44fff1a6351";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "7b6bacee-4b7a-4639-8cd3-d44fff1a6351") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "7b6bacee-4b7a-4639-8cd3-d44fff1a6351",
		rootBoundaryId
	});
}
/**
...
async function POST(request) {
	...
	try {
		...
		if (!(image instanceof File) || !title || !handle || !category || !description) return Response.json({ error: "必須項目を入力してください" }, { status: 400 });
		...
		if (![
			...
			"image/png",
			"image/webp"
		].includes(image.type) || image.size > 10 * 1024 * 1024) return Response.json({ error: "JPG・PNG・WEBP（10MB以内）を選んでください" }, { status: 400 });
		const analysis = await analyzeRoom(image, title, category, description, itemText);
		storedKey = `${crypto.randomUUID()}.${image.type.split("/")[1]}`;
		await storage.put(storedKey, image.stream(), { httpMetadata: {
